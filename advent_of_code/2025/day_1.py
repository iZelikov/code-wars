...
def get_number1(move: str, current_number: int = 50, total_numbers=100) -> int:
    n = int(move[1:]) * (-1, 1)[move[0] == 'R']
    result = current_number + n
    return result % total_numbers

def get_number2(move: str, current_number: int = 50, total_numbers=100) -> tuple[int, int]:
    if move.startswith('R'):
        number = current_number + int(move[1:])
        current_number = number % total_numbers
