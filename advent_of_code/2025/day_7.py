...
def first_part():
    with open(filename) as f:
        ...
        manifold = [list(line) for line in manifold]
        splits = 0
        for row, line in enumerate(manifold[1:]):
...
def second_part():
    with open(filename) as f:
        manifold = f.read().splitlines()
        manifold = [list(line) for line in manifold]
        print(build_timeline(manifold))

def build_timeline(manifold):
    if not manifold:
        return 1
    ...
    for row, line in enumerate(manifold[1:]):
        for col, char in enumerate(line):
            ...
            if manifold[row][col] == "|":
                if manifold[row + 1][col] == "^":
                    l_manifold = deepcopy(manifold)
                    r_manifold = deepcopy(manifold)
                    l_manifold[row + 1][col-1] = "S"
                    r_manifold[row + 1][col+1] = "S"
                    n =  build_timeline(l_manifold[row+1:]) + build_timeline(r_manifold[row+1:])
                    memo[s] = n
                    return n
                else:
                    manifold[row + 1][col] = "|"
